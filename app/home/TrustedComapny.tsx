...
const TrustedCompany: React.FC<Props> = ({ fadeIn }) => {
                ...
                {loadingTrusted ? (
                    ...
                    <div className="relative overflow-hidden py-8">
                        <div className="animate-marquee">
                            {/* Duplicate the logos 3 times for seamless loop */}
                            {[...trustedCompanies, ...trustedCompanies, ...trustedCompanies].map((company, index) => (
                                <div
                                    key={`${company._id}-${index}`}
                                    className="flex flex-col items-center justify-center min-w-[200px] h-28 px-6 opacity-70 hover:opacity-100 transition-all duration-300 hover:scale-105 flex-shrink-0"
                                >
                                    <Image
                                        src={company.image}
                                        alt={company.name}
